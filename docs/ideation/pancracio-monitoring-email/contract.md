...
**Supersedes**: contract-2026-09-26-2.html
...
- [ ] web/app.py is untouched by this project — no new backend endpoint was needed — check: `test -z "$(git diff main...HEAD -- web/app.py 2>/dev/null)"` → exits 0
...
- **web/app.py-untouched check uses `git diff main...HEAD` (this branch's own commits vs. the merge base), not a pinned historical commit SHA** — rejected: Pinning the check to a fixed baseline commit SHA captured at contract-approval time. Other sessions kept committing unrelated work to main throughout this project's execution (confirmed twice: a dashboard feature landed mid-project, then more commits landed again before Phase 3 finished), making any fixed SHA stale and the check unreliable through no fault of this project's own changes. A merge-base diff only checks what *this branch's own commits* touched, which is what the criterion actually means.
